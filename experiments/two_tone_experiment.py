...
class TwoToneExperiment(Experiment):
    ...
    def calibrate_f1_z_short(self) -> float:
        ...
        # cw_frequency = f_calib[self.find_highest_average_triplet(-20*np.log(np.abs(z_calib)))]
        cw_frequency = f_calib[self.find_highest_average_triplet(-z_calib)]

        return cw_frequency, f_calib, z_calib
    
    def calibrate_f1_z_short_masked(self, masked_frequencies) -> float:
    ...
    @Experiment.experiment_method
    def two_tone_movie_cw_voltage(self, vna_window: bool = True, live_plotting: bool=True, masked_frequencies: Optional[list] = None):
        ...
        for i, voltage in enumerate(self.config.yoko.voltages):
            ...
            if masked_frequencies is not None:
                cw_frequency, f_calib, z_calib = self.calibrate_f1_z_short_masked(masked_frequencies)
            else:
                cw_frequency, f_calib, z_calib = self.calibrate_f1_z_short()
            ...
            if live_plotting:
                self.plotter.update_twotone_imshow_movie_voltage(self.data.f1_center_frequencies/1e9,
                                                self.data.f2_center_frequencies/1e9,
                                                (self.data.f2_frequencies[i][-1] - self.data.f2_frequencies[i][0])/1e9,
                                                self.config.yoko.voltages/1e-3, self.data.signal)
    ...
    def initialize_single_shot_cw_voltage_data(self):

        sweep_length = self.config.yoko.sweep_length
        calib_points = self.config.vna_f1_calib.num_points

        calibration_zs = np.full((sweep_length, calib_points), np.nan, dtype=complex)
        calibration_f1s = np.full((sweep_length, calib_points), np.nan)

        self.data.f1_calib_zs = calibration_zs
        self.data.f1_calib_fs = calibration_f1s


        background_points = self.config.vna_cw_background.num_points

        cw_background_zs = np.full((sweep_length, background_points), np.nan, dtype=complex)
        cw_background_f1s = np.full((sweep_length, background_points), np.nan)

        self.data.cw_background_zs = cw_background_zs
        self.data.cw_background_fs = cw_background_f1s

        

    @Experiment.experiment_method
    def two_tone_single_shot_cw_voltage_aw(self, vna_window: bool = True, live_plotting: bool=True, masked_frequencies: Optional[list] = None):
        self.initialize_single_shot_cw_voltage_data()
        print('Data for single shot experiment initialized')
        cw_background_zs_rot = np.full((self.config.yoko.sweep_length, self.config.vna_cw_background.num_points), np.nan, dtype=complex)
        # check yoko current, ramp slowly to starting value
        if self.config.yoko.voltage_range >= self.yoko.voltage():
            self.yoko.set_source_voltage_sweep(self.config.yoko)
        else:
        ...
        for i, voltage in enumerate(self.config.yoko.voltages):
            ...
            self.yoko.voltage(voltage)
            self.data.voltages[i] = voltage
            #calibrate f1 frequency for this flux/voltage
            start = time.time()
            if masked_frequencies is not None:
                cw_frequency, f_calib, z_calib = self.calibrate_f1_z_short_masked(masked_frequencies)
            else:
                cw_frequency, f_calib, z_calib = self.calibrate_f1_z_short()
            print(cw_frequency)
            self.config.vna_cw_background.center_frequency = cw_frequency
            self.config.vna_f1.center_frequency = cw_frequency
            self.data.f1_center_frequencies[i] = self.config.vna_f1.center_frequency
            self.data.f1_calib_fs[i] = f_calib
            self.data.f1_calib_zs[i] = z_calib

            print(time.time() - start)
            print("calibrated")
            #take cw background
            start = time.time()
            self.vna.set_sweep(self.config.vna_cw_background)
            f, z = self.vna.sweep()
            pca = PCA(n_components=2)
            X = np.array([[np.real(z_point), np.imag(z_point)] for z_point in z])
            pca.fit(X)
            major_axis = pca.components_[0]
            print(f'CW background data major axis: {major_axis}')
            self.data.cw_background_fs[i] = f
            self.data.cw_background_zs[i] = z
            start = time.time()
            # take reference trace
            # self.vna.set_sweep(self.config.vna_f1)
            # self.vna.sweep_count = 1
            ...
            alpha = np.arctan(major_axis[1]/major_axis[0])
            z_rot = (z - np.average(np.real(z)) - 1j* np.average(np.imag(z))) * np.exp(-1j*alpha)
            try:
                self.data.S[i] = z_rot
            except:
                self.data.S[i] = np.nan
            signal = np.imag(z_rot)
            print("OK")
            #signal = S_to_dBm(z)
            self.data.signal[i] = signal
            ...
            self.data.vna_meta = vna_meta
            self.ana.output_off()
            cw_background_zs_rot[i] = (self.data.cw_background_zs[i]  - np.average(np.real(self.data.cw_background_zs[i])) - 1j* np.average(np.imag(self.data.cw_background_zs[i]))) * np.exp(-1j*alpha)
            # Save rotated background data only
            self.data.cw_background_zs[i] = cw_background_zs_rot[i]
            if live_plotting:
                self.plotter.update_twotone_imshow_single_shot_voltage(self.data.f1_center_frequencies/1e9,
                                                self.data.f2_center_frequencies/1e9,
                                                (self.data.f2_frequencies[i][-1] - self.data.f2_frequencies[i][0])/1e9,
                                                cw_background_zs_rot[i],
                                                z_rot,
                                                self.config.yoko.voltages/1e-3, self.data.signal)
    ...
    @Experiment.experiment_method
    def two_tone_single_shot_cw_voltage(self, vna_window: bool = True, live_plotting: bool=True, masked_frequencies: Optional[list] = None):
        ...
        for i, voltage in enumerate(self.config.yoko.voltages):
            ...
            # major_axis = pca.components_[0]
            z0 = np.mean(z)
            z0v = np.sqrt(np.mean((z-z0)**2))

            # print(f'CW background offset and angle: {np.abs(z0v)} ; {z0angle}')

            self.data.cw_background_fs[i] = f
            self.data.cw_background_zs[i] = (z-z0)/z0v*np.abs(z0v) #(z-z0)-z0v
            ...
            #zs = np.mean(z)
            #zsv = np.sqrt(np.mean((z-zs)**2))
            z_rot = (z-z0)/z0v*abs(z0v) # (z-zs)/zsv
            #zs = (z - z0) * np.exp(-1j*z0angle)
